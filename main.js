/*document.addEventListener("DOMContentLoaded", function () {
    ...
    const steamAPIURL = `https://34.176.188.153:3000/steamapi?steamid=${steamID}`;
    ...
    fetch(steamAPIURL)
        ...
        .then(data => {
            ...
            const steamAvatar = user ? user.avatarfull : 'https://i.pinimg.com/564x/66/c0/be/66c0bede32a10a2f017d789b259af478.jpg';
            const steamIDDisplay = user ? user.steamid : 'STEAMID_NO_DISPONIBLE';

            // Mostrar información en la página
            document.getElementById('steamAvatar').src = steamAvatar;
            document.getElementById('steamID').textContent += steamIDDisplay;
        })
        .catch(error => {
...
// Initialize circular audio visualizer without additional options
var AUDIO = AUDIO || {};
...
Visualizer.prototype.renderLounge = function () {
    ...
    for (var i = 0; i < maxBarNum; i++) {
        ...
        // Ajusta la posición en el eje Y de manera más precisa
        var y = radius - (amplitude / 12 - this.barHeight);
        ...
        this.canvasCtx.save();
        this.canvasCtx.translate(cx + this.barSpacing, cy + this.barSpacing);
        this.canvasCtx.rotate(alfa);
        this.canvasCtx.fillRect(0, y, this.barWidth, h);
        this.canvasCtx.restore();
    }
};
